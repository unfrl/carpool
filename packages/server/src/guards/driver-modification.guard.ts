...
export class DriverModificationGuard implements CanActivate {
    constructor(private readonly _carpoolService: CarpoolService, private readonly _driverService: DriverService) {}
    ...
    private async validateRequest(request: UserRequest): Promise<boolean> {
        ...
        if(carpool.user.id === request.user.id) //If the user is the creator of the carpool allow modification
        {
            return true;
        }

        return await this._driverService.findDriverIdByUserCarpoolId(request.user.id, request.params.id) != undefined;
    }
}
